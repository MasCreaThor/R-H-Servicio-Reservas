...
const seedDatabase = async () => {
    try {
        ...
        const hotelCount = await models_1.Hotel.count();
        if (hotelCount > 0) {
        ...
        // 1. Crear países
        const argentina = await models_1.Country.create({
            nombre: 'Argentina',
            codigo: 'AR'
        });
        const colombia = await models_1.Country.create({
            nombre: 'Colombia',
            codigo: 'CO'
        });
        // 2. Crear ciudades
        const buenosAires = await models_1.City.create({
            nombre: 'Buenos Aires',
            countryId: argentina.id
        });
        const bogota = await models_1.City.create({
            nombre: 'Bogotá',
            countryId: colombia.id
        });
        // 3. Crear direcciones
        const direccionBuenosAires = await models_1.Address.create({
            calle: 'Av. 9 de Julio',
            numero: '1000',
            codigoPostal: '1043',
            cityId: buenosAires.id
        });
        const direccionBogota = await models_1.Address.create({
            calle: 'Carrera 7',
            numero: '72-41',
            codigoPostal: '110231',
            cityId: bogota.id
        ...
        // 4. Crear categorías
        const lujo = await models_1.Category.create({
            nombre: 'Lujo',
            estrellas: 5,
            descripcion: 'Hoteles de lujo con todas las comodidades'
        });
        const estandar = await models_1.Category.create({
            nombre: 'Estándar',
            estrellas: 3,
            descripcion: 'Hoteles con buena relación calidad-precio'
        });
        // 5. Crear hoteles
        const hotelBuenosAires = await models_1.Hotel.create({
        ...
        const hotelBogota = await models_1.Hotel.create({
        ...
        const rooms = await models_1.Room.findAll({ attributes: ['id', 'tipo', 'hotelId'] });
